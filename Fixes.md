...
These bugs live in shared `@elvora` code, so they affect every site using this
package — port them to `master` too.
...
alias in the consuming app, this import fails to resolve at build time —
...
- `application.tsx` links to `/terms-and-conditions/` and the privacy policy
  ...
  root`) — neither page exists in `new-ignite-nursing-website` yet, so those
  links 404. Not a package bug; the consuming app just hasn't built those
  pages yet.
